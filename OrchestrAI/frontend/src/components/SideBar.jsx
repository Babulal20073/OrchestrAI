...
function SideBar() {
  ...
  //as sometime we don't have user's image from google so we need state to check if it exist
  const [imageError, setImageError] = useState(false);
  const dispatch = useDispatch()//it's for updating conversations 
  const { conversations, selectedConversation } = useSelector(state => state.conversation) //it's for getting the data of redux conversation
  ...
  //if we pressend leftpannel button
  if (collapsed) {
    return (
        ...
        <div className="relative shrink-0">
                {
                  (userData?.avatar && !imageError)
                    ...
                    border-indigo-500/25"
                      src={userData?.avatar}
                      alt={"user image"} onError={() => setImageError(true)} />
                    :
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="px-4 pt-4 pb-1">
          <button
            ...
            bg-gradient-to-br from-indigo-500 to-violet-700 rounded-xl py-[10px]
            border-none cursor-pointer
            ...
            return (
              ...
              ${isActive ? "bg-indigo-500/10 border-indigo-500/[0.18]" :
                ...
                <span className={`text-[13px] font-medium truncate 
                  ${isActive ? "text-slate-100" : "text-slate-300"}`}>{conv?.title || "New Chat"}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="px-3.5 py-3.5">
          {userData ? (
            <div className="flex items-center gap-2.5 cursor-pointer rounded-x1 px-3 py-2.5
            hover:bg-white/[0.05] transition-colors duration-150">
              ...
              <div className="flex gap-1">
                ...
                border-none bg-transparent text-yellow-600 cursor-pointer hover:bg-white/[0.8]
            ...
            text-slate-200 bg-white/[0.05] border border-white/[0.08] rounded-x1 py-[11px]
